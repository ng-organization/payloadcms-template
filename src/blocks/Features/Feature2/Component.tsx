...
import { Feature2Block as Feature2BlockProps } from '@/payload-types'
import { AnimatePresence, motion } from 'motion/react'
import { useEffect, useState } from 'react'
import Image from '@/components/PayloadImage'

export function Feature2Block({ title, autoplayInterval, features }: Feature2BlockProps) {
  const [currentFeature, setCurrentFeature] = useState(0)
  const [progress, setProgress] = useState(0)
  const featuresLength = features?.length || 0
  const autoPlayInterval = autoplayInterval || 4000
  ...
  }, [progress, featuresLength, autoPlayInterval])
